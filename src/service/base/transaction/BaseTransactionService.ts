...
export const BaseTransactionService = (prismaClient: IPrismaClientTransaction = prisma): IBaseTransactionService => {
	const service: IBaseTransactionService = {
		...AbstractRepositoryService<IBaseTransactionService>(prismaClient, prismaClient.baseTransaction, async baseTransaction => {
		...
		create: async create => prisma.$transaction(async prisma => {
			...
			return prisma.baseTransaction.create({
				data: {
					baseId: base.id,
