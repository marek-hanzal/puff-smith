...
export const CoilsJob: IJobProcessor<ICoilsJobParams> = {
	...
	handle: () => JobService().handle<ICoilsJobParams>(async ({jobProgress, job: {userId}, logger, progress}) => {
...
export const CoilJob: IJobProcessor<ICoilJobParams> = {
	...
	handle: () => JobService().handle<ICoilJobParams>(async ({jobProgress, job: {params: {wireId}, userId}, logger, progress}) => {
