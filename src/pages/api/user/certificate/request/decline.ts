...
export default MutationEndpoint<"CertificateRequestDecline", IUserCertificateRequestRequest, any>({
	handler: async ({user, request}) => UserCertificateRequestSource().withUser(user).decline(request),
});
