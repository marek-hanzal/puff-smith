...
export default MutationEndpoint<"LicenseRequestDecline", IUserLicenseRequestRequest, any>({
	handler: async ({user, request}) => UserLicenseRequestSource().withUser(user).decline(request),
});
